...
class MenuAdapter (context: Context?, val data: MutableList<SkbFunItem>) : RecyclerView.Adapter<MenuAdapter.SymbolHolder>() {
    ...
    private fun isSettingsMenuSelect(data: SkbFunItem): Boolean {
        ...
        val result: Boolean = when (data.skbMenuMode) {
            ...
            SkbMenuMode.PinyinNormal17 -> rimeValue == CustomConstant.SCHEMA_ZH_DOUBLE_NORMAL17
            SkbMenuMode.Pinyin26Double -> rimeValue.startsWith(CustomConstant.SCHEMA_ZH_DOUBLE_FLYPY) && rimeValue !in setOf(CustomConstant.SCHEMA_ZH_DOUBLE_LX17, CustomConstant.SCHEMA_ZH_DOUBLE_NORMAL17)
