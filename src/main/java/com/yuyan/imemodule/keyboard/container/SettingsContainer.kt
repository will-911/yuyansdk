...
class SettingsContainer(context: Context, inputView: InputView) : BaseContainer(context, inputView) {
    ...
    fun showSkbSelelctModeView() {
        ...
        funItems.add(
            SkbFunItem(
                mContext.getString(R.string.keyboard_name_pinyin_normal_17),
                R.drawable.selece_input_mode_lx17,
                SkbMenuMode.PinyinNormal17
            )
        )
        funItems.add(
            SkbFunItem(
    ...
    private fun onKeyboardMenuClick(data: SkbFunItem) {
        val value = when (data.skbMenuMode) {
            ...
            SkbMenuMode.PinyinNormal17 -> Pair(InputModeSwitcher.MASK_SKB_LAYOUT_NORMAL17, CustomConstant.SCHEMA_ZH_DOUBLE_NORMAL17)
